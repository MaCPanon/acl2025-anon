...
class MaCPLayer(BaseTunerLayer):
    ...
    def update_layer(self, adapter_name, n_frequency, scale, init_MaCP_weights=None):
        if n_frequency <= 0:
    ...
    def reset_MaCP_parameters(self, adapter_name, init_MaCP_weights):
        if init_MaCP_weights is False:
            return

        if adapter_name in self.spectrum.keys():
            if init_MaCP_weights is True:
                nn.init.kaiming_uniform_(self.spectrum[adapter_name].weight, a=math.sqrt(5))
            elif init_MaCP_weights.lower() == "gaussian":
                nn.init.normal_(self.spectrum[adapter_name].weight, std=1 / self.r[adapter_name])
            else:
                raise ValueError(f"Unknown initialization {init_MaCP_weights=}")
        if adapter_name in self.spectrum.keys():
...
class Linear(nn.Module, MaCPLayer):
    def __init__(
        self,
        base_layer,
        adapter_name: str,
        ...
        init_MaCP_weights: Union[bool, str] = True,
        **kwargs,
    ) -> None:
        super().__init__()
        MaCPLayer.__init__(self, base_layer, **kwargs)
        self.fan_in_fan_out = fan_in_fan_out

        self._active_adapter = adapter_name
        self.update_layer(adapter_name, n_frequency, scale, init_MaCP_weights)
